...
// std::transform cannot determine which overload of tolower()
// and toupper() to use, the one from ctype.h or the one from std
// namespace. To disambiguate, we wrap the calls.
...
{
	transform(str.begin(), str.end(), str.begin(), toupper_wrapper);
	return str;
}
