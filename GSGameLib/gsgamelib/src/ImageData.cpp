...
{
	SMART_ASSERT(strFileName.size()>0).msg("File name string is empty");
...
void ImageData::AddAlphaChannel(const Color3UB& color/*=Color3UB::Black()*/)
{
...
{
	SMART_ASSERT(iWidthScaleFactor>0 && iHeightScaleFactor>0)
		(iWidthScaleFactor)(iHeightScaleFactor).msg("Scale factors must be positive");
	...
	pSrc = pDst = NULL; // No longer valid, don't use these anymore
